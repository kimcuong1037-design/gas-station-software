...
```bash
# 前置：Python 3.11+（检查：python3 --version）

# 1. 进入后端目录（B0 完成后才有此目录）
cd backend
...
# → Swagger UI 在 http://localhost:5000/api/docs/
...
> **注意：** 后端目录（`backend/`）在 B0 基础设施搭建完成后才存在。如果现在没有，先认领 B0 任务（`MODULE-ASSIGNMENTS.md §2 B0 部分`）。
...
| 9 | ★☆☆ | `docs/ROADMAP.md` | 全景图：7 个阶段 26 个模块 + 后端 B0/B1 里程碑 | 10 min |
| 10 | ★☆☆ | `docs/STANDARDS.md §8-10` | **后端开发者专读**：后端编码规范、数据库规范、测试规范 | 15 min |
...
| 架构 | `docs/features/energy-trade/price-management/architecture.md` | 实体定义→API 端点→状态机→业务规则→MySQL 8.0 Schema 的完整链路 |
...
> 前提：B0 基础设施已搭建完成，`backend/` 目录存在。

以 Module 1.1 站点管理为后端走读范例：

| 步骤 | 文件 | 观察重点 |
|------|------|---------|
| 架构参考 | `docs/features/operations/station/architecture.md §DB Schema` | SQLAlchemy Model 的字段来源 |
| Model | `backend/app/models/station.py` | 字段定义、审计字段、软删除、关系声明 |
| Schema | `backend/app/schemas/station.py` | 序列化字段、输入验证规则 |
| Service | `backend/app/services/station_service.py` | 业务规则实现、分页逻辑 |
| Blueprint | `backend/app/api/stations.py` | 端点定义、flask-smorest 装饰器用法 |
| 测试 | `backend/tests/test_stations.py` | 集成测试结构；`tests/unit/` 单元测试 |
| 迁移文件 | `backend/migrations/versions/` | 查看最新迁移文件了解迁移格式 |

### 走读后自测

完成走读后，你应能回答以下问题：
1. 一个模块从需求到上线，要产出哪些文档和代码文件？
2. `architecture.md` 和 `types.ts` / `models/` 是什么关系？谁是真相来源？
3. 新前端模块上线需要同步修改哪些共享文件？
4. 新后端模块需要修改哪些共享文件？
5. RequirementTag 三步注册流程是什么？
6. 侧边栏为什么是三级结构而不是两级？
7. 后端为什么 Service 层不能依赖 Flask request 对象？
