...
对于依赖外部模块数据的功能，必须评估是否需要在本模块中提供 UI 访问入口：

| 依赖数据 | 来源模块 | 本模块是否需要入口 | 入口形式 |
|---------|---------|------------------|----------|
| {{数据名称}} | {{模块名}} | 是/否 | 子菜单/快捷链接/内嵌组件 |

**判断依据：**
- 如果本模块的核心流程**强依赖**该数据（如交接班依赖排班表确定接班人），则**必须**提供入口
- 如果仅为偶尔参考（如查看员工详情），可以不提供但建议添加快捷链接
- 不能简单标注"属于其他模块"就忽略用户在本模块中的访问需求

**示例：** 排班表数据虽然逻辑上属于站点管理，但交接班模块中应提供"排班表"子菜单，因为：
1. 站长在交接班场景下需频繁查看/调整排班
2. 站点概况页需要展示当前/下一班次的排班信息
3. 发起交接班时需要知道下一班的接班人
