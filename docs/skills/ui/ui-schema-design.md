...
1. 所有页面跳转必须引用**页面清单中定义的路由路径**，禁止自行发明路径
2. 路由路径的唯一真实来源是「页面清单」表格中的 `路由` 列
3. 交互行为表格中必须使用完整路径，不允许只写页面编号（如 P01）
4. 动态路由参数（如 `:id`）必须明确说明数据来源

**示例：**

| 触发 | 行为 | 路由路径 | 说明 |
|------|------|----------|------|
| 点击 [新增] | 跳转新增页面 | `/operations/station/new` | 空表单 |
| 点击 [查看] | 跳转详情页面 | `/operations/station/:id` | id 来自当前行数据 |
| 点击 [编辑] | 跳转编辑页面 | `/operations/station/:id/edit` | id 来自当前行数据 |
...
- [ ] 所有页面都有明确的路由定义
- [ ] **路由一致性：所有交互跳转的路径与页面清单中的路由完全一致**
- [ ] **无硬编码路径：交互行为中无自行发明的路径（如 `/create` vs 定义的 `/new`）**
- [ ] 列表页包含筛选/搜索功能
