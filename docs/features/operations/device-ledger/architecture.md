...
不同设备类型的专有属性通过 `type_specific_fields` (JSON) 存储，避免宽表。
...
| `remark` | `TEXT` | | 备注 |
| `type_specific_fields` | `JSON` | | 设备类型专有字段（见 §1.2） |
...
不同设备类型在 `type_specific_fields` JSON 字段中存储专有属性：
...
| `nozzle_data` | `JSON` | | 枪位实时数据（加气机）：`[{nozzleNo, fuelType, status, unitPrice, currentFlow}]` |
| `extra_data` | `JSON` | | 其他扩展监控数据 |
| `updated_at` | `TIMESTAMP` | NOT NULL, DEFAULT NOW() | 更新时间 |
...
| D4 | 设备类型专有字段使用 JSON | 不同设备类型的专有属性差异大，JSON 避免宽表，保持灵活性 |
...
```sql
-- ============================================================
-- 设备设施管理模块 — MySQL 8.0 Database Schema
...
CREATE TABLE equipment (
  ...
  device_type           ENUM('tank', -- 储罐 'dispenser', -- 加气机 'pump', -- 泵 'valve', -- 阀门 'sensor', -- 传感器 'fire_equipment', -- 消防设备 'electrical' -- 电气设备) NOT NULL,
  model                 VARCHAR(100),
  manufacturer          VARCHAR(100),
  serial_number         VARCHAR(100),
  station_id            CHAR(36)        NOT NULL,    -- FK → Station（跨模块，应用层保证一致性）
  location              VARCHAR(255),
  install_date          DATE,
  status                ENUM('active', -- 正常运行 'fault', -- 故障 'pending_maintenance', -- 待维保 'inactive' -- 已停用) NOT NULL DEFAULT 'active',
  maintenance_cycle     ENUM('daily', 'weekly', 'monthly', 'quarterly', 'semi_annual', 'annual'),
  ...
  created_by            CHAR(36),                    -- FK → User（跨模块，应用层保证一致性）
  updated_by            CHAR(36),                    -- FK → User（跨模块，应用层保证一致性）

  CONSTRAINT uq_equipment_device_code UNIQUE (device_code)
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_unicode_ci COMMENT = '设备台账 — 记录每台设备的完整档案信息，核心实体';
...
CREATE TABLE equipment_photo (
  id            CHAR(36) PRIMARY KEY DEFAULT (UUID()),
  equipment_id  CHAR(36)         NOT NULL REFERENCES equipment (id) ON DELETE CASCADE,
  ...
  uploaded_by   CHAR(36)                             -- FK → User（跨模块，应用层保证一致性）
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_unicode_ci COMMENT = '设备照片 — 每台设备最多 10 张，格式 JPG/PNG，单张 ≤ 5 MB';
...
CREATE TABLE equipment_monitoring (
  ...
  dispenser_status      ENUM('idle', 'fueling', 'fault', 'offline'),
  daily_volume          DECIMAL(10,2),
  sensor_value          DECIMAL(10,4),
  sensor_unit           VARCHAR(20),
  connection_status     ENUM('connected', -- 已连接 'disconnected', -- 已断开 'unstable' -- 不稳定) NOT NULL DEFAULT 'disconnected',
...
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_unicode_ci COMMENT = '设备实时监控数据 — 每台设备一条记录（1:1），持续高频更新';
...
-- Table comment: equipment_monitoring_log
-- 设备监控历史日志 — 定时快照用于趋势图。
-- 此表为时间序列数据，MySQL: use PARTITION BY RANGE for time-series optimization
...
CREATE TABLE maintenance_order (
  ...
  station_id         CHAR(36)          NOT NULL,    -- FK → Station（跨模块，应用层保证一致性）
  order_type         ENUM('maintenance', -- 保养 'repair', -- 维修 'report' -- 报修)    NOT NULL,
  urgency            ENUM('low', -- 低 'medium', -- 中 'high', -- 高 'urgent' -- 紧急) NOT NULL DEFAULT 'medium',
  status             ENUM('pending', -- 待处理 'processing', -- 处理中 'pending_review', -- 待验收 'completed', -- 已完成 'closed' -- 已关闭)  NOT NULL DEFAULT 'pending',
  description        TEXT          NOT NULL,
  assignee_id        CHAR(36)          NOT NULL,    -- FK → Employee（跨模块，应用层保证一致性）
  ...
  created_by         CHAR(36)          NOT NULL,    -- FK → User（跨模块，应用层保证一致性）
  created_at         TIMESTAMP     NOT NULL DEFAULT CURRENT_TIMESTAMP,
  updated_at         TIMESTAMP     NOT NULL DEFAULT CURRENT_TIMESTAMP,

  CONSTRAINT uq_maintenance_order_no UNIQUE (order_no)
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_unicode_ci COMMENT = '维保工单 — 设备维护核心实体，支持保养/维修/报修三种类型';
...
CREATE TABLE order_record (
  id          CHAR(36) PRIMARY KEY DEFAULT (UUID()),
  order_id    CHAR(36)          NOT NULL REFERENCES maintenance_order (id) ON DELETE CASCADE,
  action      ENUM('created', -- 创建工单 'started', -- 开始处理 'submitted_review', -- 提交验收 'approved', -- 验收通过 'rejected', -- 退回处理 'closed', -- 关闭工单 'record_added' -- 添加处理记录) NOT NULL,
  content     TEXT          NOT NULL,
  parts       VARCHAR(500),
  cost        DECIMAL(10,2),
  duration    DECIMAL(8,2),
  operator_id CHAR(36)          NOT NULL,    -- FK → User（跨模块，应用层保证一致性）
  created_at  TIMESTAMP     NOT NULL DEFAULT CURRENT_TIMESTAMP
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_unicode_ci COMMENT = '工单处理记录 — 工单处理过程的流水记录，形成完整处理时间线';
...
CREATE TABLE order_attachment (
  id          CHAR(36) PRIMARY KEY DEFAULT (UUID()),
  record_id   CHAR(36)         NOT NULL REFERENCES order_record (id) ON DELETE CASCADE,
  file_url    VARCHAR(500) NOT NULL,
  file_name   VARCHAR(255) NOT NULL,
  file_size   INTEGER,
  mime_type   VARCHAR(50),
  ...
  uploaded_at TIMESTAMP    NOT NULL DEFAULT CURRENT_TIMESTAMP,
  uploaded_by CHAR(36)                             -- FK → User（跨模块，应用层保证一致性）
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_unicode_ci COMMENT = '工单附件 — 工单处理记录的附件（故障照片、维修凭证等）';
...
CREATE TABLE maintenance_plan (
  ...
  equipment_id   CHAR(36)              NOT NULL REFERENCES equipment (id) ON DELETE CASCADE,
  station_id     CHAR(36)              NOT NULL,    -- FK → Station（跨模块，应用层保证一致性）
  frequency      ENUM('daily', -- 日检 'weekly', -- 周检 'monthly', -- 月检 'quarterly', -- 季检 'semi_annual', -- 半年检 'annual' -- 年检) NOT NULL,
  ...
  updated_at     TIMESTAMP         NOT NULL DEFAULT CURRENT_TIMESTAMP,
  created_by     CHAR(36)                           -- FK → User（跨模块，应用层保证一致性）
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_unicode_ci COMMENT = '保养计划 [MVP+] — 周期性保养计划模板，到期自动创建维保工单';
...
CREATE TABLE alarm_rule (
  ...
  alarm_level   ENUM('info', -- 信息 'warning', -- 警告 'critical' -- 严重) NOT NULL DEFAULT 'warning',
  enabled       BOOLEAN     NOT NULL DEFAULT TRUE,
  created_at    TIMESTAMP   NOT NULL DEFAULT CURRENT_TIMESTAMP,
  updated_at    TIMESTAMP   NOT NULL DEFAULT CURRENT_TIMESTAMP,
  created_by    CHAR(36)                     -- FK → User（跨模块，应用层保证一致性）
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_unicode_ci COMMENT = '告警规则 [MVP+] — 设备监控指标的告警阈值配置';
...
CREATE TABLE alarm_record (
  id              CHAR(36) PRIMARY KEY DEFAULT (UUID()),
  rule_id         CHAR(36)          REFERENCES alarm_rule (id) ON DELETE SET NULL,
  equipment_id    CHAR(36)          NOT NULL REFERENCES equipment (id) ON DELETE CASCADE,
  station_id      CHAR(36)          NOT NULL,    -- FK → Station（跨模块，应用层保证一致性）
  alarm_level     ENUM('info', -- 信息 'warning', -- 警告 'critical' -- 严重)   NOT NULL,
  alarm_type      ENUM('level_low', -- 液位过低 'pressure_high', -- 压力过高 'temperature_abnormal', -- 温度异常 'gas_leak', -- 气体泄漏 'device_offline', -- 设备离线 'other' -- 其他)    NOT NULL,
  message         TEXT          NOT NULL,
  metric_value    DECIMAL(10,4),
  status          ENUM('unhandled', -- 未处理 'acknowledged', -- 已确认 'resolved' -- 已解决)  NOT NULL DEFAULT 'unhandled',
  acknowledged_by CHAR(36),                      -- FK → User（跨模块，应用层保证一致性）
  acknowledged_at TIMESTAMP,
  resolved_by     CHAR(36),                      -- FK → User（跨模块，应用层保证一致性）
  resolved_at     TIMESTAMP,
  resolution_note TEXT,
  triggered_at    TIMESTAMP     NOT NULL DEFAULT CURRENT_TIMESTAMP
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_unicode_ci COMMENT = '告警记录 [MVP+] — 告警事件日志';
