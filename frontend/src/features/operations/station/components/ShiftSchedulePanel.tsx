...
const ShiftSchedulePanel: React.FC<ShiftSchedulePanelProps> = ({ stationId }) => {
  ...
  const stationShifts = shifts.filter(s => s.stationId === stationId);
  const stationEmployees = employees.filter(e => e.stationId === stationId && e.status === 'active');
  ...
  const weekSchedules = useMemo(() => {
    ...
    return localSchedules.filter(
      s => s.stationId === stationId && s.date >= start && s.date <= end
    );
  }, [localSchedules, stationId, weekDates]);
  ...
  const handleSave = () => {
    form.validateFields().then(values => {
      ...
      message.success(t('station.schedule.saveSuccess'));
  ...
  const columns = [
    {
      title: t('station.schedule.shift'),
      dataIndex: 'shiftName',
      key: 'shiftName',
      width: 120,
      fixed: 'left' as const,
        ...
        render: (entry: ShiftScheduleEntry | undefined, record: Record<string, unknown>) => {
          ...
          return (
            <div style={{ textAlign: 'center' }}>
              ...
              >
                {t('station.schedule.addSchedule')}
              </Button>
            </div>
          );
        },
  ...
  return (
    <div>
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 16 }}>
        <Title level={5} style={{ margin: 0 }}>{t('station.schedule.title')}</Title>
        <Space>
          <Button icon={<LeftOutlined />} onClick={() => setWeekOffset(w => w - 1)}>
            {t('station.schedule.prevWeek')}
          </Button>
          <Button type={weekOffset === 0 ? 'primary' : 'default'} onClick={() => setWeekOffset(0)}>
            {t('station.schedule.thisWeek')}
          </Button>
          <Button onClick={() => setWeekOffset(w => w + 1)}>
            {t('station.schedule.nextWeek')} <RightOutlined />
          </Button>
        </Space>
      </div>
      ...
      <Modal
        title={editingEntry ? t('station.schedule.editSchedule') : t('station.schedule.addSchedule')}
        open={editModalOpen}
        onOk={handleSave}
      ...
      >
        <Form form={form} layout="vertical">
          ...
          <Form.Item
            name="shiftId"
            label={t('station.schedule.shift')}
            rules={[{ required: true }]}
          >
            <Select
          ...
          <Form.Item
            name="employeeId"
            label={t('station.schedule.employee')}
            rules={[{ required: true }]}
          >
            <Select
